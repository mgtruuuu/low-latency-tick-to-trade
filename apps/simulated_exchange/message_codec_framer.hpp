 ...
 * @brief TLV message framer for TCP frame boundary detection.
 *
 * Adapts message_codec.hpp (TLV wire format) for streaming TCP recv buffers.
 *
 ...
 * like the gateway recv thread to receive complete TLV messages and
 * parse them independently. The framer's job is purely frame boundary
 * detection — the handler owns message interpretation.
 *
 * Similarly, encode() is a pass-through memcpy because the gateway
 * send thread already produces fully TLV-wrapped response bytes.
 */
...
struct MessageCodecFramer {
  ...
  decode(std::span<const std::byte> buf) const noexcept {
    if (buf.size() < net::kMessageHeaderSize) {
      return {.status = net::FrameStatus::kIncomplete,
              .frame_size = 0,
              .payload = {}};
    }
    ...
    if (magic != net::kMessageMagic) {
      return {
          .status = net::FrameStatus::kError, .frame_size = 0, .payload = {}};
    }

    // Read payload_len from the header (offset 8, 4 bytes BE).
    const auto plen = static_cast<std::size_t>(sys::load_be32(buf.data() + 8));
    ...
    return {.status = net::FrameStatus::kOk,
            .frame_size = frame_size,
            .payload = buf.subspan(0, frame_size)};
  }
