...
auto make_md_buf(std::uint64_t seq, std::uint32_t symbol_id, mk::algo::Side side,
                 mk::algo::Price price, mk::algo::Qty qty,
                 std::int64_t ts = 0) {
...
TEST_F(FeedHandlerTest, ParseValid) {
  ...
  ASSERT_TRUE(
      handler_.on_udp_data(reinterpret_cast<const char *>(buf.data()),
                           buf.size(), out));
...
TEST_F(FeedHandlerTest, SequentialSequence) {
  ...
  for (std::uint64_t seq = 1; seq <= 3; ++seq) {
    ...
    EXPECT_TRUE(handler_.on_udp_data(
        reinterpret_cast<const char *>(buf.data()), buf.size(), out));
  }
...
TEST_F(FeedHandlerTest, GapDetection) {
  ...
  ASSERT_TRUE(handler_.on_udp_data(
      reinterpret_cast<const char *>(buf1.data()), buf1.size(), out));

  // Seq 5 — gap of 3 (missed seq 2, 3, 4).
  auto buf5 = make_md_buf(5, 1, mk::algo::Side::kBid, 10000, 100);
  ASSERT_TRUE(handler_.on_udp_data(
      reinterpret_cast<const char *>(buf5.data()), buf5.size(), out));
...
TEST_F(FeedHandlerTest, DuplicateRejection) {
  ...
  ASSERT_TRUE(handler_.on_udp_data(
      reinterpret_cast<const char *>(buf1.data()), buf1.size(), out));
  ASSERT_TRUE(handler_.on_udp_data(
      reinterpret_cast<const char *>(buf2.data()), buf2.size(), out));
...
TEST_F(FeedHandlerTest, CounterConsistency) {
  ...
  EXPECT_FALSE(handler_.on_udp_data(
      reinterpret_cast<const char *>(dup.data()), dup.size(), out));
