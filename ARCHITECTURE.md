...
- **Tick-to-Trade p50 components are consistent with the kernel-bound interpretation.** Components are measured on different sample populations (RX kernel path is all-tick paired delta; Tick-to-Trade and Order Send path are sent-order only), so their p50s are not strictly additive — but the magnitudes line up:
  ...
  The ~4.5 µs gap should not be read as a measured standalone stage. One plausible explanation — not yet confirmed by direct measurement — is that these p50 values come from different sample populations: RX kernel path is measured on **all ticks**, while Tick-to-Trade and TSC inner span are measured **only on sent-order ticks**. Sent-order ticks fire when `spread > spread_threshold` (see `spread_strategy.hpp`), and if wider spreads correlate with bursty market-data conditions on the rig, their kernel RX/wakeup delay may be larger than the all-tick RX KernelGap p50. That correlation is a working hypothesis, not a property the spread strategy enforces directly. Additional contributors independent of that hypothesis include a batch-position gap between the batch-level `userspace_recv_ns` timestamp and the per-message `t0` rdtsc point, and instrumentation overhead (`rdtsc`, `clock_gettime`, bookkeeping); their relative sizes have not been measured directly. A sent-order-only RX KernelGap histogram would be required to isolate this — captured in `TODO.md` as a post-interview observability item. Note that **Order Send path is not a pure TX kernel measurement** — it also includes risk check, payload serialization, message framing, and post-send result/log handling executed before the timestamp; a strictly TX-kernel-only stage would require dedicated rdtsc points immediately before and after `send_nonblocking()`.
...
- **Feed Parse and Strategy are pure CPU work** (~100-300 ns) — parsing a fixed 34-byte datagram and evaluating a spread condition. Independent of network I/O by construction (no syscalls, no socket operations on these measured paths).
